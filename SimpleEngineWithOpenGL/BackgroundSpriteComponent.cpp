...
#include "Actor.h"
#include "../SimpleEngineWithSDL/Rectangle.h"
...
void BackgroundSpriteComponent::setScrollSpeed(float scroolSpeedP)
{
	scrollSpeed = scroolSpeedP;
}
...
void BackgroundSpriteComponent::update(float dt)
{
	...
	for (auto& bg : textures)
	{
		...
		//If this is completly off the screen reset offset to the right of the last bg texture
